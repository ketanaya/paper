...
を使用し、将来の値を予測する。ここで、ARIMAモデルについての定義を行う。そのために必要な\emph{差分化}、
\emph{自己回帰モデル}、\emph{移動平均モデル}についても定義を行う。
\subsection{差分化}
\subsection{自己回帰モデル}
\subsection{移動平均モデル}
\subsection{ARIMAモデル}
差文化、自己回帰モデル、移動平均モデルを組み合わせると、非季節性ARIMAモデル(AutoRegressive Integrated Moving Average)
となる。ARIMAモデルのモデル式は下記のようにかくことができる。\\
$y'_t = y_t - y_{t-1}$とするとき
...
\end{equation} 
ただし$y'_t$は一回を超えて差分化される場合もある。\ref{model:arima}式を$ARIMA(p,d,q)$モデルと呼ぶ。
なお, これらは \cite{Hyndman} を参照している。

\section{モデル化の手順}
非季節性時系列データにARIMAモデルを適合する際は、下記の手順を実施する。
