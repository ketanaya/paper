...
データにトレンドがある場合、観測時点が近い観測値はその値も近いため、小さいラグでの自己相関係数は大きな正値になる可能性がたかい。
...
一般的に弱定常をもつ時系列のことを定常な時系列と呼び、本研究での定常は弱定常のことを指す。また、上記の定義に当てはまらない定常ではない時系列を
...
\label{fig:predict} 
\end{figure}
この結果から、日経平均株価は将来に渡って、上昇傾向にあることが予測される。
